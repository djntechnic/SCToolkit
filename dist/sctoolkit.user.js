...
(() => {
    ...
    formatLogUrl(url = "") {
      ...
      try {
        const parsed = new URL(
          url,
          typeof window !== "undefined" && window.location ? window.location.href : "https://www.tcdb.com"
        );
  ...
  var DEFAULT_CONFIG = {
    ...
    global: {
      ...
      addMultiplesPauseDurationMs: 3e3,
...
`;
  ...
  function initInputOptimization() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (index < inputs.length - 1) {
        ...
        const submitBtn = form ? form.querySelector('button[type="submit"]') || form.querySelector('input[type="submit"]') || form.querySelector(".btn-primary") : typeof document !== "undefined" ? document.querySelector('button[type="submit"], input[type="submit"], .btn-primary') : null;
  ...
  var ICONS = {
    ...
    calendar: {
      size: 12,
      strokeWidth: 2,
      body: '<rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><path d="M16 2v4"/><path d="M8 2v4"/><path d="M3 10h18"/>'
    },
    pin: {
      size: 12,
      strokeWidth: 2,
  ...
  // src/net/googleSheets.js
  function sanitizeSheetId(inputStr) {
  ...
  async function sendToGoogleSheet({ sheetId, worksheet, tsvLine, webAppUrl }) {
    ...
    const payload = {
      ...
      rawTsv: tsvLine,
      targetCol: 9
      // Explicitly target column I (column 9)
    };
    Log(
  ...
  function resolveYearFromDocument(setName = "", doc = typeof document !== "undefined" ? document : null) {
    if (doc) {
      ...
      if (yearBreadcrumb) {
        ...
        const match2 = href.match(/\/year\/([^/?#]+)/i);
        if (match2) return decodeURIComponent(match2[1]);
  ...
  var SHORTCUT_KEYS = ["CHECKLIST", "YEAR", "INSERTS", "PARALLELS", "FOR_SALE", "MULTI", "WANTLIST"];
  var TOOLBAR_BADGES = ["CHECKLIST", "PIN", "YEAR", "INSERTS", "PARALLELS", "FOR_SALE", "MULTI", "WANTLIST", "CSV", "HIERARCHY"];
  var SET_LINK_BADGES = ["CHECKLIST", "PIN", "CSV", "HIERARCHY", "YEAR", "INSERTS", "PARALLELS", "FOR_SALE", "MULTI", "WANTLIST"];
  ...
  function createBadge(badgeKey, sid = null, onClickOverride = null, displayMode = "both", parentSid = null, overrides = {}) {
    const config = BADGES[badgeKey];
    if (!config) return null;
    const textLabel = overrides.text !== void 0 && overrides.text !== null && overrides.text !== "" ? overrides.text : config.text;
...
`;
  ...
  function getToolbarTitle(doc = document) {
    ...
    if (!title || genericLabels.includes(title.toLowerCase())) {
      const setHeader = doc.querySelector?.("#setname-content h1") || doc.querySelector?.("#main-content-area h2") || doc.querySelector?.("#main-content-area h1") || doc.querySelector?.("h1.site");
      const subHeader = doc.querySelector?.("#setname-content h3") || doc.querySelector?.("#main-content-area h3");
      if (setHeader && !setHeader.textContent.toLowerCase().includes("set links") && !setHeader.textContent.toLowerCase().includes("change log")) {
        title = setHeader.textContent.replace(/\s*-\s*Cards$/i, "").trim();
      }
      if (subHeader && title && !title.toLowerCase().includes(subHeader.textContent.trim().toLowerCase())) {
  ...
  function isActionLink2(a) {
    const href = a?.href || "";
    return href.includes("CollectionAdd") || href.includes("CollectionEdit") || href.includes("CollectionWant") || href.includes("CollectionStatus") || href.includes("CollectionMove") || href.includes("CollectionRemove") || href.includes("SalesLinks") || href.includes("MODE=") || href.includes("Team.cfm") || href.includes("Person.cfm");
  }
  function extractCardNo(el) {
    if (!el) return "";
    const cardLinks = Array.from(
      el.querySelectorAll('a[href*="ViewCard.cfm"], a[href*="/cid/"], a[href*="CollectionCard.cfm"]')
    ).filter(
      (a) => !a.querySelector("img") && !a.querySelector("i") && a.textContent.trim().length > 0 && !isActionLink2(a)
    );
    if (cardLinks.length > 0) {
  ...
  function applyFilter(index, term) {
    ...
    index.forEach(({ el, haystack, cardNo, cleanCardNo }) => {
      const rowCleanCardNo = cleanCardNo !== void 0 ? cleanCardNo : cardNo ? cardNo.replace(/^#/, "").trim().toLowerCase() : extractCardNo(el)?.replace(/^#/, "").trim().toLowerCase() || "";
      const match = conditions.length === 0 || conditions.some((cond) => {
  ...
  `;
    ...
    if (copyFriendlyBtn) {
      copyFriendlyBtn.addEventListener("click", () => {
        ...
        const writePromise = win.navigator?.clipboard?.writeText ? win.navigator.clipboard.writeText(fullText) : Promise.resolve();
        writePromise.then(() => {
  ...
  function disconnectChecklistEnhancer() {
  ...
  function observeChecklistFilter(options = {}) {
    ...
    const observer = new MutationObserver((mutations) => {
      const isSelfMutation = mutations.every(
        (m) => Array.from(m.addedNodes).every(
          (node) => node.nodeType === 1 && (node.id === "tk-checklist-filter-wrap" || node.querySelector?.("#tk-checklist-filter-wrap") !== null)
        )
      );
  ...
  var ActiveObservers2 = /* @__PURE__ */ new Set();
  function disconnectSetListEnhancer() {
    ActiveObservers2.forEach((obs) => {
      try {
        obs.disconnect();
      } catch {
      }
    });
    ActiveObservers2.clear();
  }
      ...
      if (options.timeoutMs > 0) {
        setTimeout(() => {
          ...
          try {
            ...
            ActiveObservers2.delete(observer);
          }
        }, options.timeoutMs);
      }
    } catch (err) {
  ...
  var STORAGE_BATCH_KEY = "sctoolkit_add_multiples_batch";
  var USER_INTENT_EVENTS = ["keydown", "pointerdown", "wheel"];
  var isSubmitting = false;
  var currentDirtyState = { isDirty: false, distinctRows: 0, totalQuantity: 0 };
  var activePauseInterval = null;
  ...
  function getPageIndex(url = typeof window !== "undefined" ? window.location.href : "", root = document) {
    if (url) {
  ...
  function getNextPageUrl(currentPageIndex, root = document, baseUrl = typeof window !== "undefined" ? window.location.href : "") {
  ...
  function getDirtyQuantityInputs(root = document) {
  ...
  function getDirtyCardDetails(root = document) {
  ...
  function getDirtyState(root = document) {
    const dirtyInputs = getDirtyQuantityInputs(root);
    let totalQty = 0;
  ...
  function setupDirtyTracking(root = document) {
    if (!root || typeof root.addEventListener !== "function") return;
    const updateState = () => {
      const nextState = getDirtyState(root);
      if (nextState.isDirty !== currentDirtyState.isDirty || nextState.distinctRows !== currentDirtyState.distinctRows || nextState.totalQuantity !== currentDirtyState.totalQuantity) {
  ...
  function setupBeforeUnloadWarning(win = typeof window !== "undefined" ? window : null, root = document) {
    const targetWin = win || root && root.defaultView || (typeof window !== "undefined" ? window : null);
  ...
  function setupSubmitHandler(root = document) {
    ...
    form.addEventListener("submit", (e) => {
      ...
      e.preventDefault();
      e.stopPropagation();
      const pauseMs = Config.global?.addMultiplesPauseDurationMs ?? 3e3;
      if (pauseMs <= 0) {
  ...
  function startPauseCountdown(form, dirtyState, pauseMs, root) {
    ...
    let remainingSec = Math.ceil(remainingMs / 1e3);
  ...
  `;
    ...
    activePauseInterval = setInterval(() => {
      remainingMs -= 1e3;
      remainingSec = Math.ceil(remainingMs / 1e3);
      if (remainingSec > 0) {
  ...
  function checkAndHandlePostReloadSuccess(root = document) {
    if (!root || typeof root.querySelector !== "function") return;
    let batchData = null;
    const storage = getStorage(root);
    try {
    ...
    const currentUrl = targetWin && targetWin.location ? targetWin.location.href : "";
    const currentPageIndex = getPageIndex(currentUrl, root);
    let successAlert = root.querySelector("#content > div.col-md-6.nopadding > div > div.alert.alert-success") || root.querySelector("#content .alert-success") || root.querySelector(".alert.alert-success");
    if (!successAlert) {
  ...
  function showAddedDetailsModal(batchData, root = document) {
  ...
  function focusFirstQuantityField(root = typeof document !== "undefined" ? document : null) {
    if (!root) return;
  ...
  function getSubmitButton(root = typeof document !== "undefined" ? document : null) {
  ...
  function setupLastRowEnterHandler(root = typeof document !== "undefined" ? document : null) {
    if (!root || typeof root.addEventListener !== "function") return;
  ...
  function initAddMultiplesEnhancer(root = typeof document !== "undefined" ? document : null) {
    if (!root) return;
  ...
  function getCollectionCardDetails(root = document) {
    let cardRows = Array.from(root.querySelectorAll("tr.collection_row"));
    if (cardRows.length === 0) {
      cardRows = Array.from(
        root.querySelectorAll("#main-content-area table tr, #content table tr")
  ...
  function showCollectionQuantityDetailsModal(counts = {}, root = document) {
  ...
  // src/modules/quickAddGridEnhancer.js
  function getListContext(pathname = window.location.pathname) {
    const lowerPath = pathname.toLowerCase();
    if (lowerPath.includes("wantlist")) return "W";
    if (lowerPath.includes("forsaletrade")) return "S";
    return "G";
  }
  function getContextSetId(pathname = window.location.pathname, search = window.location.search) {
    const pathMatch = pathname.match(/\/sid\/(\d+)/i);
    if (pathMatch) return pathMatch[1];
    const urlParams = new URLSearchParams(search);
    return urlParams.get("SetID") || "";
  }
  function getSportType(search = window.location.search) {
  ...
  function injectRowQuickAdd(row, context = {}) {
    ...
    container.style.cssText = "display: inline-flex !important; flex-direction: row !important; align-items: center !important; flex-wrap: nowrap !important; margin-top: 4px !important; margin-bottom: 2px !important; width: max-content !important; min-width: 0 !important; max-width: 100% !important; vertical-align: middle !important;";
    const qtyInput = document.createElement("input");
    qtyInput.type = "number";
    qtyInput.className = "tk-qty-input";
    qtyInput.value = "1";
    qtyInput.min = "1";
    qtyInput.style.cssText = "width: 28px !important; min-width: 28px !important; max-width: 28px !important; flex: 0 0 28px !important; height: 22px !important; text-align: center !important; border: 1px solid var(--tk-border-strong, #ccc) !important; border-right: none !important; border-radius: 3px 0 0 3px !important; font-size: 11px !important; font-family: monospace !important; padding: 0 !important; margin: 0 !important; outline: none !important; background: var(--tk-bg-elevated, #ffffff) !important; color: var(--tk-text, #000000) !important; box-sizing: border-box !important; line-height: 20px !important; -moz-appearance: textfield;";
    const actionBtn = document.createElement("button");
    actionBtn.type = "button";
    actionBtn.className = "tk-add-btn";
    actionBtn.textContent = "+";
    actionBtn.style.cssText = "width: 24px !important; min-width: 24px !important; max-width: 24px !important; flex: 0 0 24px !important; height: 22px !important; background: var(--tk-accent, #0d6efd) !important; color: #ffffff !important; border: 1px solid var(--tk-accent, #0d6efd) !important; border-radius: 0 3px 3px 0 !important; padding: 0 !important; margin: 0 !important; font-size: 12px !important; font-weight: 700 !important; cursor: pointer !important; display: inline-flex !important; align-items: center !important; justify-content: center !important; box-sizing: border-box !important; line-height: 1 !important; user-select: none !important;";
  ...
  // src/modules/collectionAddCardNumberEnhancer.js
  var BREADCRUMB_SELECTOR = "#content > div.col-md-6.nopadding > div > div.d-none.d-md-block > nav > ol > li:nth-child(3)";
  var FALLBACK_BREADCRUMB_SELECTOR = 'nav[aria-label="breadcrumb"] ol.breadcrumb > li:nth-child(3)';
  var ADD_TO_SELECT_SELECTOR = 'form[name="CFForm_1"] select[name="AddTo"], #CFForm_1 select[name="AddTo"]';
  var TEXTAREA_SELECTOR = 'form[name="CFForm_1"] textarea[name="sText"], #CFForm_1 textarea[name="sText"]';
  var FORM_SELECTOR = 'form[name="CFForm_1"], #CFForm_1';
  function defaultAddToSelect(root = document) {
  ...
  function checkCollectionMismatch(root = document, preferredId = getDefaultCollectionId()) {
    const result = {
      isMismatch: false,
    ...
    if (result.pageCollectionId && result.pageCollectionId !== result.preferredCollectionId) {
      ...
      if (!existingWarning && form) {
        ...
        <div>
          <strong>Collection Mismatch Warning:</strong> Current collection (<strong>${escapeHtml3(collectionName)}</strong>) does not match your preferred collection (ID: <strong>${escapeHtml3(result.preferredCollectionId)}</strong>).
        </div>
      `;
        form.parentNode.insertBefore(warningDiv, form);
      }
      showToast({
        message: `<b>Collection Warning:</b> Current collection ("${escapeHtml3(collectionName)}") does not match your preferred collection.`,
        variant: "warn",
        location: "bottom-right"
      });
    }
  ...
  function countTextareaCards(text = "") {
  ...
  function validateCardNumbers(text = "") {
    const rawLines = text.split(/\r?\n/);
    const errors = [];
  ...
  function updateCardCounterWidget(counts, root = document) {
  ...
  function initCollectionAddCardNumberEnhancer() {
    ...
    if (actions.validateInput !== false && form && textarea) {
      form.addEventListener("submit", (e) => {
        ...
        if (!validation.isValid) {
          ...
          const errorItems = validation.errors.map((err) => `<li>${escapeHtml3(err.message)}</li>`).join("");
          alertDiv.innerHTML = `
  ...
  var SettingsUI = {
    ...
    _buildBadgesPane: () => {
      ...
      hint.textContent = "Configure toolbar action badges, set-link badges, and center-toolbar hotlinks (Top, Bottom, Search, or custom links). Click section headers to expand/collapse. Each item allows custom text, tooltip, launch target mode, and placement ordering.";
      ...
      const buildCollapsibleSection = (sectionTitle, configKey, onApply, isHotlinkSection = false) => {
        ...
        const updateCount = () => {
          ...
          countBadge.textContent = `${activeCount}/${entries.length} active`;
        };
        const flush = () => {
          SettingsUI._persist();
          updateCount();
        ...
        const rebuild = () => {
          ...
          currentEntries.forEach((entry, idx) => {
            ...
            const displayName = isHotlinkSection ? entry.text || entry.id || `Hotlink ${idx + 1}` : badgeDef.title || entry.key;
            ...
            textInput.type = "text";
            textInput.value = isHotlinkSection ? entry.text || "" : entry.text !== void 0 ? entry.text : badgeDef.text || "";
            ...
            tooltipInput.value = isHotlinkSection ? entry.tooltip || "" : entry.tooltip !== void 0 ? entry.tooltip : badgeDef.title || "";
            ...
            fieldsRow.appendChild(targetGroup);
    ...
    _buildGlobalPane: () => {
      ...
      popoverDurationInput.value = Config.global.cardFormatterPopoverDurationMs || 4e3;
